...
in order to keep minions on their toes. But you've noticed a flaw in the
...
def solution(n, b):
    def flawed_algo(n, b):
        # single-step in the flawed algorithm
        k = len(n)
        y = [int(i) for i in n]
        y.sort()
        x = y[::-1]

        z = int("".join(str(i) for i in x), b) \
          - int("".join(str(i) for i in y), b)

        out = "" # int to given base
        for i in range(k)[::-1]:
            out += str(z // b**i)
            z = z % b**i
        return out
    ...
    indices = [] # remember seen indices
    while(True):
        n = flawed_algo(n, b)
        if n in indices:
            return indices.index(n) + 1
        else:
            indices.insert(0, n)